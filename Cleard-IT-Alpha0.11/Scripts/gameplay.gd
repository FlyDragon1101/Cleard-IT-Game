...
const fall_distance = [1, 1, 1, 1, 2, 2, 3, 3, 4, 4, 5, 6, 7, 8, 9, 24]
...
func move_contol(motion):
	if not_colliding_x(motion) and not_colliding_y(motion):
		...
		if motion == Vector2i.DOWN and lock_delay == 0:
			...
			game_over(position_now.x, position_now.y)
			held = false
...
func clear_line(lines=0):
	...
	while rows > 0:
		...
		if solid_blocks == width:
			floor_rows(rows)
			Game.total_lines_cleared += 1
			lines += 1
			level_up(Game.level, Game.total_lines_cleared)
			is_line_clear = true
		else:
			rows -= 1
	...
	if is_line_clear:
		if position_now.y <= 9:
			Game.score += int(clears[line_clears[lines]] * Game.level * 1.5)
			$ScoreLog.text += "\n" + lc[lines] + " (+" + str(int(clears[line_clears[lines]] * Game.level * 1.5))+ ")"
		#if get_spin_bonus:
			#score += clears[line_clears[lines]] * 3
			#message = spin_ident(message, lines)
			#$ScoreLog.text += "\n" + message + " (+" + str(clears[line_clears[lines]] * level * 3)+ ")"
		else:
			Game.score += clears[line_clears[lines]] * Game.level
			$ScoreLog.text += "\n" + lc[lines] + " (+" + str(clears[line_clears[lines]] * Game.level)+ ")"
		if leveled_up:
			$ScoreLog.text += "\n LEVEL UP! " + str(Game.level-1)+ " -> " + str(Game.level)
	
	$Stats.text = "Score: " + str(Game.score)
	$Lines.text = "Lines cleared: " + str(Game.total_lines_cleared)
	$Level.text = "Level: " + str(Game.level) + "/50"
	if Game.score > 1:
		$HighScore.text = "High score: " + str(Game.score)
...
func game_over(x, y):
	for block in active_piece:
		if y + block.y < 4 and x + block.x >3 and x + block.x < 7 and not not_colliding_y(block):
			...
			$"GameOver/Game over".text = "GAME OVER!\nClick anywhere to exit.\nYour score is: " + str(Game.score)
			Game.score_log = $ScoreLog.text
	return true


func _input(button):
	if not running and button.is_action_pressed("ui_select"):
		get_tree().change_scene_to_file("res://scenes//post_game.tscn")
	...
	if Input.is_action_just_pressed("Toggle") and panel_shown:
		panel_shown = not panel_shown
		$Controls.hide()
	elif Input.is_action_just_pressed("Toggle") and not panel_shown:
		panel_shown = not panel_shown
		$Controls.show()
...
func _physics_process(_delta):
	if running:
		...
		if Game.level < 35 and not_colliding_y(motion):
			level_speed = frame_table[Game.level-1]
		elif Game.level >= 35 and not_colliding_y_harddrop(motion):
			level_speed = frame_table[-1]
			change_tiles()
			
			for i in range(fall_distance[Game.level-35]):
				position_now.y += 1
				for block in active_piece:
					if position_now.y + block.y + motion.y > height or not not_colliding_y(motion):
						position_now.y -= 1
		...
		if Input.is_action_pressed("Soft Drop") and inc % 2 == 0 and position_now.y < height and Game.level <= 29:
			motion = Vector2i.DOWN
			if not_colliding_y(Vector2i.DOWN):
				Game.score += 1
			$Stats.text = "Score: " + str(Game.score)
		
		if Input.is_action_just_pressed("Hard Drop") and Game.level <= 34:
			hard_drop(motion)
			inc = level_speed
			lock_delay = 0
